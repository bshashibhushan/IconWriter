...
package com.ikon.module.jcr.stuff;
...
public class RepositoryAccessManager implements AccessManager {
	...
	public void init(AMContext context) throws AccessDeniedException, Exception {
		...
		for (Iterator<java.security.Principal> it = subject.getPrincipals().iterator(); it.hasNext();) {
			...
			} else if (obj instanceof com.ikon.jaas.RoleImpl) {
				com.ikon.jaas.RoleImpl rol = (com.ikon.jaas.RoleImpl) obj;
				log.debug("Rol: {}", rol.getName());
				principalRoles.add(rol.getName());
						...
						if ((permissions & Permission.READ) != 0) {
							...
							try {
								access = checkProperties(node, 
										com.ikon.bean.Permission.USERS_READ,
										com.ikon.bean.Permission.ROLES_READ);
							...
							try {
								access = checkProperties(node, 
										com.ikon.bean.Permission.USERS_WRITE,
										com.ikon.bean.Permission.ROLES_WRITE);
							} catch (PathNotFoundException e) {
							...
							try {
								access = checkProperties(node, 
										com.ikon.bean.Permission.USERS_DELETE,
										com.ikon.bean.Permission.ROLES_DELETE);
							} catch (PathNotFoundException e) {
								log.debug("{} PropertyNotFoundException({}) in {}", new Object[] {
										subject.getPrincipals(), e.getMessage(),
										node.getPrimaryNodeType().getName() });
							...
							try {
								access = checkProperties(node, 
										com.ikon.bean.Permission.USERS_SECURITY,
										com.ikon.bean.Permission.ROLES_SECURITY);
							} catch (PathNotFoundException e) {
								log.debug("{} PropertyNotFoundException({}) in {}", new Object[] {
										subject.getPrincipals(), e.getMessage(),
										node.getPrimaryNodeType().getName() });
	...
	private boolean checkProperties(Node node, String userProperty, String roleProperty) throws 
		...
		// Propiedad no definida en nt:versionHistory, nt:version y openkm:resource
