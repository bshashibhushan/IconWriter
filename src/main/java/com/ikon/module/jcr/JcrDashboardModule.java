...
public class JcrDashboardModule implements DashboardModule {
	...
	public List<DashboardDocumentResult> getUserLockedDocuments(Session session) throws
		...
		String qs = "/jcr:root/"+Repository.ROOT+"//element(*, openkm:document)[@jcr:lockOwner='"+session.getUserID()+"' and openkm:content/@jcr:isCheckedOut=false()]";
	...
	public List<DashboardDocumentResult> getUserCheckedOutDocuments(Session session) throws 
		...
		String qs = "/jcr:root/"+Repository.ROOT+"//element(*, openkm:document)[@jcr:lockOwner='"+session.getUserID()+"' and openkm:content/@jcr:isCheckedOut=true()]";
	...
	public List<DashboardDocumentResult> getUserSubscribedDocuments(Session session) throws 
		...
		String qs = "/jcr:root/"+Repository.ROOT+"//element(*, mix:notification)[@jcr:primaryType='openkm:document' and @openkm:subscriptors='"+session.getUserID()+"']";
	...
	public List<DashboardFolderResult> getUserSubscribedFolders(Session session) throws
		...
		String qs = "/jcr:root/"+Repository.ROOT+"//element(*, mix:notification)[@jcr:primaryType='openkm:folder' and @openkm:subscriptors='"+session.getUserID()+"']";
		...
		try {
			...
			for (Iterator<Object[]> it = q.iterate(); it.hasNext() && cont < MAX_RESULTS; cont++) {
				...
				try {
					...
					// if (node.getPath().startsWith("/openkm:root")) {
						Document doc = BaseDocumentModule.getProperties(session, node);
						DashboardDocumentResult vo = new DashboardDocumentResult();
						vo.setDocument(doc);
						vo.setDate(resDate);
