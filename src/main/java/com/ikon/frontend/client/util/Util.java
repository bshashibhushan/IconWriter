...
public class Util {
	...
	/**
	 * Document List to Excel
	 */
	public static void exportToExcel(String path, String params) {
		if (!params.equals("") && !params.endsWith("&")) {
			params += "&";
		}
		
		final Element downloadIframe = RootPanel.get("__download").getElement(); 
		String url = RPCService.ExportToExcelServlet + "?" + params + "id=" + URL.encodeQueryString(path);
		DOM.setElementAttribute(downloadIframe, "src", url); 
	}
	...
	public static native void createPDFViewerFlexPaper(String pdfUrl, String width, String height, boolean printEnabled) /*-{
		if (printEnabled) {
