...
import com.ikon.frontend.client.extension.widget.userinfo.UserInfoExtension;
...
import com.ikon.frontend.client.widget.chat.ChatRoomDialogBox;
import com.ikon.frontend.client.widget.chat.ChatRoomPopup;
import com.ikon.frontend.client.widget.chat.OnlineUsersPopup;
import com.ikon.frontend.client.service.OKMWorkflowService;
import com.ikon.frontend.client.service.OKMWorkflowServiceAsync;
...
public class UserInfo extends Composite {
	...
	private final OKMWorkflowServiceAsync workflowService = (OKMWorkflowServiceAsync) GWT.create(OKMWorkflowService.class);//this create a class,object 
	...
	public UserInfo() {
		...
		imgWorkflowTasks.addClickHandler(new ClickHandler() {
			@Override
			public void onClick(ClickEvent event) {
				String url = Main.get().workspaceUserProperties.getWorkspace().getWorkflowUrl();
				if ((!("").equals(url))) {
					Window.open(url + "/jw/web/console/run/inbox", "_blank", null);
				} else {
					Window.alert("Please ask your administrator to configure workflow url.");
				}
	...
	public void setLockedDocuments(int value) {
		...
		String usr = Main.get().workspaceUserProperties.getUser().getId(); 
		workflowService.getUserTaskLists(usr, callback);
	}
	...
	final AsyncCallback<Integer> callback = new AsyncCallback<Integer>() { 	 	
						...
						public void onSuccess(Integer result) { 	
							...
							newWorkflowTasks.setHTML("&nbsp;" + result + "&nbsp;"); 	 	
