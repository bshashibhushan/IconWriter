...
import com.ikon.frontend.client.extension.event.hashandler.HasWorkspaceHandlerExtension;
...
public class TabWorkspace extends Composite implements HasWorkspaceEvent, HasWorkspaceHandlerExtension {
		...
		tabBar.addSelectionHandler(new SelectionHandler<Integer>() {
			...
			public void onSelection(SelectionEvent<Integer> event) {
				...
				switch (index) {
					...
					default :
						loginJS(GeneralComunicator.getUser(), Main.get().workspaceUserProperties.getWorkspace().getWorkflowUrl());
	...
	public Widget getWidgetExtensionByIndex(int index) {
		return (Widget) widgetExtensionList.get(0);
	}
	...
	public static native void loginJS(String user, String url) /*-{ 
	  var callback = "";
	  $wnd.AssignmentManager.login(url + "/jw", user, user, callback)
	}-*/;
}
