...
package com.ikon.frontend.client.widget.filebrowser.menu;
...
import com.ikon.frontend.client.widget.MenuBase;
...
public class TaxonomyMenu extends MenuBase {
	...
	public TaxonomyMenu() {
		...
		annotate = new MenuItem(Util.menuHTML("img/icon/actions/annotate.gif", Main.i18n("Annotate")), true, annotateFile);
		annotate.addStyleName("okm-MenuItem");
		dirMenu.addItem(annotate);
		...
		exportToExcel = new MenuItem(Util.menuHTML("img/icon/actions/exportToExcel.gif", Main.i18n("Export to Excel")), true, exportToExcelHandler);
		exportToExcel.addStyleName("okm-MenuItem-strike");
		dirMenu.addItem(exportToExcel);
	...
	// Command menu to Export document list to Excel
	Command exportToExcelHandler = new Command() {
		public void execute() {		
			if (toolBarOption.exportOption) {
				Main.get().mainPanel.desktop.browser.fileBrowser.exportToExcel();
				hide();
			}
		}
	};
	...
	// Command menu to download file
	Command annotateFile = new Command() {
		public void execute() {		
			final Element downloadIframe = RootPanel.get("__download").getElement();
			
			//generate swf file
			String url1 = RPCService.ConverterServlet + "?inline=true&toSwf=true&uuid=" + FileBrowserComunicator.getDocument().getUuid();
			DOM.setElementAttribute(downloadIframe, "src", url1); 
			
			String url = RPCService.AnnotateServlet + "?action=getAnnotation&uuid=" + FileBrowserComunicator.getDocument().getUuid();
	...
	public void langRefresh() {
		...
		annotate.setHTML(Util.menuHTML("img/icon/actions/annotate.gif", Main.i18n("Annotate")));
		...
		exportToExcel.setHTML(Util.menuHTML("img/icon/actions/exportToExcel.gif", Main.i18n("Export to Excel")));
	}
	...
	public void evaluateMenuOptions(){
		...
		if (toolBarOption.checkoutOption){enable(annotate);} else {disable(annotate);}
		...
		if (toolBarOption.exportOption){enable(export);enable(exportToExcel);} else {disable(export);disable(exportToExcel);}
		if (toolBarOption.bookmarkOption){enable(bookmark);} else {disable(bookmark);}
	}
	...
	public void setAvailableOption(GWTAvailableOption option) {
		...
		if (!option.isAnnotateOption()) {
			dirMenu.removeItem(annotate);
		}
		...
		if (!option.isExportOption()) {
			...
			dirMenu.removeItem(exportToExcel);
		}
	}
