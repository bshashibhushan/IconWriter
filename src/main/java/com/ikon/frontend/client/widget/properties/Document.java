...
public class Document extends Composite {
	...
	private RetentionPolicyPopup retentionPopup;
	...
	public void set(final GWTDocument doc) {
		...
		//add retentionpolicy
		addButton = new Button(GeneralComunicator.i18n("Set"), new ClickHandler(){
		     public void onClick(ClickEvent event) {
		        	retentionPopup = new RetentionPolicyPopup(doc.getUuid(), doc.getPath());
		      		retentionPopup.setWidth("300px");
		      		retentionPopup.setHeight("100px");
		      		retentionPopup.setStyleName("okm-Popup");
		      		retentionPopup.addStyleName("okm-DisableSelect");
		      		retentionPopup.show();
		     }
		 });
		addButton.setStyleName("okm-Button");
		...
		deleteButton = new Button(GeneralComunicator.i18n("button.delete"), new ClickHandler(){
		     public void onClick(ClickEvent event) {
		    	 Main.get().confirmPopup.setConfirm(ConfirmPopup.CONFIRM_DELETE_RETENTION_POLICY);
		...
		if(doc.getPolicy() == null){
			if(remove)
				tableProperties.setWidget(11, 1, addButton);
			else 
			    tableProperties.setHTML(11, 1, "never");
		    tableProperties.setHTML(11, 2, " ");
		} else {
		    tableProperties.setHTML(11, 1, doc.getPolicy().getExpiryDate() + "( " + doc.getPolicy().getDestinationPath() + " )");
		    if(remove)
		    	tableProperties.setWidget(11, 2, deleteButton);
		    else
			    tableProperties.setHTML(11, 2, " ");
		}
		...
		if (actualView == UIDesktopConstants.NAVIGATOR_TRASH) {
			...
			tableProperties.getCellFormatter().setVisible(11,1,false);
			tableProperties.getCellFormatter().setVisible(11,2,false);
		} else {
			...
			tableProperties.getCellFormatter().setVisible(11,1,true);
			tableProperties.getCellFormatter().setVisible(11,2,true);
		}
