...
import com.ikon.frontend.client.extension.event.hashandler.HasFolderHandlerExtension;
import com.ikon.frontend.client.extension.event.hashandler.HasMailHandlerExtension;
...
import com.ikon.frontend.client.service.OKMMassiveServiceAsync;
import com.ikon.frontend.client.service.OKMNotifyService;
import com.ikon.frontend.client.service.OKMNotifyServiceAsync;
...
import com.ikon.frontend.client.widget.filebrowser.menu.CategoriesMenu;
import com.ikon.frontend.client.widget.filebrowser.menu.MailMenu;
import com.ikon.frontend.client.widget.filebrowser.menu.PersonalMenu;
import com.ikon.frontend.client.widget.filebrowser.menu.TaxonomyMenu;
import com.ikon.frontend.client.widget.filebrowser.menu.TemplatesMenu;
import com.ikon.frontend.client.widget.filebrowser.menu.ThesaurusMenu;
import com.ikon.frontend.client.widget.filebrowser.menu.TrashMenu;
...
public class FileBrowser extends Composite implements OriginPanel, HasDocumentEvent, HasFolderEvent, HasMailEvent,
	...
	private com.ikon.frontend.client.widget.popup.Status massiveStatus;
	...
	public void restore() {
		if (table.isDocumentSelected() && table.getDocument() != null) {
			Main.get().activeFolderTree.folderSelectPopup1.setEntryPoint(FolderSelectPopup.ENTRYPOINT_BROWSER);
			Main.get().activeFolderTree.folderSelectPopup1.setToRestore(table.getDocument());
			Main.get().activeFolderTree.showDirectorySelectPopup1();
		} else if (table.isFolderSelected() && table.getFolder() != null) {
			Main.get().activeFolderTree.folderSelectPopup1.setEntryPoint(FolderSelectPopup.ENTRYPOINT_BROWSER);
			Main.get().activeFolderTree.folderSelectPopup1.setToRestore(table.getFolder());
			Main.get().activeFolderTree.showDirectorySelectPopup1();
		} else if (table.isMailSelected() && table.getMail() != null) {
			Main.get().activeFolderTree.folderSelectPopup.setEntryPoint(FolderSelectPopup.ENTRYPOINT_BROWSER);
	...
	/**
	 * Export document list of a folder to excel
	 */
	public void exportToExcel() {
		if (table.isFolderSelected()) {
			Util.exportToExcel(getFolder().getPath(), "exportToExcel");
		}
	}
	...
	/**
	 * purgeMassive to purge large number of elements
	 */
	
	public void purgeMassive() {
		massiveService.purge(Main.get().mainPanel.desktop.browser.fileBrowser.getAllSelectedPaths(),
				new AsyncCallback<Object>() {
				@Override
				public void onSuccess(Object result) {
					Main.get().mainPanel.topPanel.toolBar.executeRefresh();
