...
public class ErrorPopup extends DialogBox implements ClickHandler {
	...
	private VerticalPanel vPanelServer;
	private HTML text;
	...
	public ErrorPopup() {
		...
		vPanelServer = new VerticalPanel();
		vPanelServer.getElement().getStyle().setBackgroundColor("#888888");
		vPanelServer.setWidth("225px");
		vPanelServer.setHeight("10px");
		
		text= new HTML();	
		vPanelServer.add(new HTML("<br>"));
		vPanelServer.add(text);
		vPanelServer.add(new HTML("<br>"));
		vPanelServer.add(button);
		vPanelServer.add(new HTML("<br>"));
		
		text.setHorizontalAlignment(HasHorizontalAlignment.ALIGN_CENTER);	
		vPanelServer.setCellHorizontalAlignment(button, VerticalPanel.ALIGN_CENTER);

		button.setStyleName("okm-YesButton");

		hide();
		setWidget(vPanelServer);
	}
